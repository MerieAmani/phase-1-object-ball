...
bigShoeRebounds = () => {
  ...
  for (const team in game) {
    for (const player in game[team].players) {
      if (game[team].players[player].shoe > playerWithLargestShoe.shoe) {
...
mostPointsScored = () => {
  ...
  for (const team in game) {
    for (const player in game[team].players) {
      if (game[team].players[player].points > playerWithMostPoints.points) {
        playerWithMostPoints = game[team].players[player];
      }
    }
  }
